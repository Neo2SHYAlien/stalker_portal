...
class TvChannels extends LibBase{
    private static $instance = null;
    protected static $collection_fields = array(
        'id', 'locked', 'number', 'logo', 'name', 'enable_tv_archive', 'cmd', 'monitoring_status', 'enable_monitoring',
        'monitoring_status_updated', 'xmltv_id', 'status', 'tv_genre_id'
    );

    protected static $exemplar_fields = array(
        'id', 'name', 'number', 'use_http_tmp_link', 'wowza_tmp_link', 'nginx_secure_link', 'enable_tv_archive', 'wowza_dvr',
        'flussonic_dvr', 'nimble_dvr', 'censored', 'base_ch', 'bonus_ch', 'hd', 'cost', 'cmd', 'cmd_1', 'cmd_2', 'cmd_3',
        'mc_cmd', 'enable_wowza_load_balancing', 'allow_pvr', 'allow_local_pvr', 'allow_local_timeshift', 'enable_monitoring',
        'descr', 'tv_genre_id', 'status', 'xmltv_id', 'service_id', 'volume_correction', 'correct_time', 'modified',
        'added', 'tv_archive_duration', 'logo'
    );

    protected static $dependents = array('claims', 'tvgenres', 'tvlogos', 'tvchlinks');
    ...
    public function getCollection(){
        $this->getChannels();
        if ($this->request->getAction() == 'get') {
            $this->setChannelLogo()->setApiLinks(self::$dependents);
        }
    ...
    private function getChannels(){
        Mysql::getInstance()->from('itv');
        if ($this->request->getAgent()) {
            $fields = $this->request->getAction() == 'get' ? self::$exemplar_fields: 'id';
            $this->setQueryExemplarParams();
        } else {
            $fields = $this->request->getAction() == 'get' ? self::$collection_fields: 'id';
    ...
    private function setChannelLogo(){
        $logo_dir = Config::get('portal_url') . 'misc/logos';
        $this->setResult(array_map(function($row) use ($logo_dir){
            $logo = $row['logo'];
            $row['logo'] = array_map(function($resolution) use ($logo, $logo_dir){
                return $logo_dir . '/' . $resolution . '/' . $logo;
            }, array( 320=>320, 240=>240, 160=>160, 120=>120));
            return $row;
        }, $this->getResult()));
        return $this;
    }
    ...
    public function deleteCollection() {
        ...
        if (($collection = $this->getCollection()->getResult())){
            $request = $this->request;
            $request->setParent('/' . $this->request->getResource() . '/' . $this->request->getAgent());
            $request->setAgent('');
            $request->setResource('claims');
            $deleted['claims'] = Claims::getInstance()->setRequest($request)->deleteCollection()->getResult();

            while(($exemplar = array_shift($collection))){
                Mysql::getInstance()->insert('deleted_channels', array('ch_id' => (int) $exemplar['id'], 'deleted' => 'NOW()'));
                $deleted['tvchannels'] += Mysql::getInstance()->delete('itv', array('id' => $exemplar['id']))->total_rows();
            }
        }
    ...
    public function updateCollection(){
        ...
        if (($collection = $this->getCollection()->getResult())){
            $fields = array_fill_keys(self::$exemplar_fields, 0);
            unset($fields['id']);
            unset($fields['logo']);
            $data = array_intersect_key($this->request->getPut(), $fields);

            while(($exemplar = array_shift($collection))){
                $updated['tvchannels'] += Mysql::getInstance()->update('itv', $data, array('id' => $exemplar['id']))->total_rows();
            }
        }
...
class Itv_Old /*extends AjaxResponse implements \Stalker\Lib\StbApi\Itv*/ {
